...
int main(int argc, char **argv){
    ...
    *argv[1] = 0;
    ...
    printf("const u32 %s_width = %i;\n", name, w);
    printf("const u32 %s_height = %i;\n", name, h);
    printf("const u16 %s_data = {\n", name);
    ...
    for(y = 0; y < h; y++){
        for(x = 0; x < w; x++){
            ...
            unsigned short highcolor = (imageData[(((y*h) + x) * 4)] >> 3) << 1;
            highcolor |= (imageData[(((y*h) + x) * 4) + 1] >> 3) << 6;
            highcolor |= (imageData[(((y*h) + x) * 4) + 2] >> 3) << 11;
            highcolor |= (imageData[(((y*h) + x) * 4) + 3] & 0X01);

            printf("0x%.4X, ", highcolor);
        
            if((y+1 != h || x+1 != w) && ((y*w)+x+1) % 16 == 0)
                printf("\n\t");
        }
    }
